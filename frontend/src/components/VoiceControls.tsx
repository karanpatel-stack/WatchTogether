...
export default function VoiceControls() {
  const { isMuted, isInVoice, speakingUsers, toggleMute, joinVoice, leaveVoice, voiceSettings } = useVoice()

  const speakingCount = speakingUsers.size

  return (
    <div className="absolute bottom-3 left-1/2 -translate-x-1/2 z-10 flex items-center gap-2 px-3 py-2 rounded-2xl bg-black/60 backdrop-blur-xl border border-white/[0.08] shadow-[0_4px_24px_rgba(0,0,0,0.4)]">
      {isInVoice ? (
        <>
          {/* Mute/Unmute */}
          <button
